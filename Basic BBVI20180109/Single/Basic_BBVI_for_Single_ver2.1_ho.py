...
#init_lambda_mu = tf.convert_to_tensor(np.array(rand.multivariate_normal(mean=[0.1, 0.0], cov=[[0.1, 0.0], [0.0, 0.1]]), dtype=np.float32))
...
init_lambda_Lambda = tf.constant([[0.5, 0.0], [0.0, 1.0]], dtype=tf.float32)
#_init_lambda_nu = np.array(rand.uniform(low=D-1+0.001, high=30.0, size=1), dtype=np.float32)
#_init_lambda_nu = np.array(rand.uniform(low=9.0, high=11.0, size=1), dtype=np.float32)
#init_lambda_nu = tf.convert_to_tensor(_init_lambda_nu)[0]
init_lambda_nu = tf.constant(8.0)
...
rho = tf.minimum(init_rho, tf.divide(tf.ones(1), tf.multiply(update_counter_ass, tf.constant(100000.0))))
...
with tf.name_scope('Prepare_for_output'):
    ...
    Area_covariance_ellipse = tf.reduce_prod(eig_Sigma, axis=1)
