...
#init_lambda_pi = tf.constant(0.5, shape=[K])
...
with tf.name_scope('LogDistributions'):
    ...
    log_p = tf.add(log_p_x, log_p_pi, name='log_p')
...
for epoch in range(num_epochs):
    ...
    result = sess.run([sample_q_mu_ass, sample_Sigma, sample_q_z_ass, sample_q_pi_ass, lambda_pi_cared,
                       log_p, log_q, lambda_pi_update, rho_ass, update_counter_ass,
                       grad_q_pi, sample_mean_pi, delta_pi, log_loss, log_p_x,
