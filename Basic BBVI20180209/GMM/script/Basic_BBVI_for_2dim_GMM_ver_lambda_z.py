...
init_lambda_z = tf.constant(np.array(rand.dirichlet(alpha=[0.1, 0.5], size=N), dtype=np.float32))
#init_lambda_z = tf.constant(np.array([0.9, 0.1]*np.array(np.ones(shape=[N, K]), dtype=np.float32), dtype=np.float32))
#init_lambda_pi = tf.constant(np.array(rand.gamma(shape=5.0, scale=2.0, size=K), dtype=np.float32))
init_lambda_pi = tf.constant([0.7, 0.3])
...
with tf.name_scope('LogDistributions'):
    ...
    log_p = tf.add(log_p_x, log_p_z, name='log_p')
...
for epoch in range(num_epochs):
    ...
    result = sess.run([sample_q_mu_ass, sample_Sigma, sample_q_z_ass, sample_q_pi_ass, lambda_z_cared,
                       log_p, log_q, lambda_z_update, rho_ass, update_counter_ass,
                       grad_q_z, sample_mean_z, delta_z, log_loss, log_p_x,
    ...
    #series9 = pd.Series([result[4][0], result[4][1]], index=data_frame9.columns)
    ...
    elbo = cal_elbo.ELBO(result[5], result[6])
    series11 = pd.Series([np.mean(result[5]), np.mean(result[6]), elbo], index=data_frame11.columns)
    data_frame11 = data_frame11.append(series11, ignore_index=True)

    series12 = pd.Series([np.mean(result[16], axis=0)[0], np.mean(result[16], axis=0)[1]], index=data_frame12.columns)
    data_frame12 = data_frame12.append(series12, ignore_index=True)
    
    if epoch == num_epochs-1:
        for n in range(N):
            series13 = pd.Series([result[17][n][0][0], result[17][n][0][1], result[17][n][1][0], result[17][n][1][1]], index=data_frame13.columns)
            data_frame13 = data_frame13.append(series13, ignore_index=True)

    for n in range(N):
        series14 = pd.Series([np.mean(result[10], axis=0)[0][n][0], np.mean(result[10], axis=0)[0][n][1]], index=data_frame14.columns)
        data_frame14 = data_frame14.append(series14, ignore_index=True)

    #series15 = pd.Series([np.mean(result[10], axis=0)[0][0], np.mean(result[10], axis=0)[0][1]], index=data_frame15.columns)
