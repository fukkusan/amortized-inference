...
#init_lambda_mu = tf.convert_to_tensor(np.array(rand.multivariate_normal(mean=[1.5, 1.5], cov=[[0.1, 0.0], [0.0, 0.1]]), dtype=np.float32))
#init_lambda_mu = tf.constant([1.0, 1.0])
init_lambda_mu = tf.convert_to_tensor(np.array(rand.multivariate_normal(mean=[1.5, 1.5], cov=[[0.1, 0.0], [0.0, 0.1]]), dtype=np.float32))
#init_lambda_muLambda = tf.constant([[0.3, 0.0], [0.0, 0.3]])
init_lambda_muLambda = tf.multiply(tf.convert_to_tensor(np.array(rand.uniform(low=3.3, high=3.8, size=1), dtype=np.float32)), unit_matrices)
#init_lambda_Lambda = tf.convert_to_tensor(np.array(stats.wishart.rvs(df=20.0, scale=[[0.5, 0.0], [0.0, 0.5]]), dtype=np.float32))
#init_lambda_Lambda = tf.constant([[0.3, 0.0], [0.0, 0.3]], dtype=tf.float32)
#init_lambda_Lambda = tf.contrib.linalg.LinearOperatorDiag(tf.matrix_diag_part(tf.convert_to_tensor(np.array(stats.wishart.rvs(df=10.0, scale=[[0.5, 0.0], [0.0, 0.5]]), dtype=np.float32)))).to_dense()
init_lambda_Lambda = tf.multiply(tf.convert_to_tensor(np.array(rand.uniform(low=0.75, high=0.8, size=1), dtype=np.float32)), unit_matrices)
...
init_lambda_nu = tf.constant(5.0)
...
with tf.name_scope('Prepare_for_output'):
    ...
    expect_val_q_Lambda = tf.multiply(lambda_nu, lambda_Lambda)
    inv_expect_val_q_Lambda = tf.matrix_inverse(expect_val_q_Lambda)
    eig_inv_expect_val_q_Lambda = tf.self_adjoint_eigvals(inv_expect_val_q_Lambda)
...
data_frame8 = pd.DataFrame(index=[], columns=['grad_q_Lambda_el11', 'grad_q_Lambda_el12', 'grad_q_Lambda_el21', 'grad_q_Lambda_el22'])
data_frame9 = pd.DataFrame(index=[], columns=['sample_mean_Lambda_el11', 'sample_mean_Lambda_el12', 'sample_mean_Lambda_el21', 'sample_mean_Lambda_el22'])
data_frame10 = pd.DataFrame(index=[], columns=['delta_Lambda_el11', 'delta_Lambda_el12', 'delta_Lambda_el21', 'delta_Lambda_el22'])
data_frame11 = pd.DataFrame(index=[], columns=['inv_expect_val_q_Lambda_el11', 'inv_expect_val_q_Lambda_el12', 'inv_expect_val_q_Lambda_el21', 'inv_expect_val_q_Lambda_el22'])
data_frame12 = pd.DataFrame(index=[], columns=['eig_inv_expect_val_q_Lambda_eig1', 'eig_inv_expect_val_q_Lambda_eig2'])
data_frame13 = pd.DataFrame(index=[], columns=['eig_val_lambda_Lambda_cared_eig1', 'eig_val_lambda_Lambda_cared_eig2'])
data_frame14 = pd.DataFrame(index=[], columns=['grad_q_mu_element1', 'grad_q_mu_element2'])
data_frame15 = pd.DataFrame(index=[], columns=['grad_q_muLambda_el11', 'grad_q_muLambda_el12', 'grad_q_muLambda_el21', 'grad_q_muLambda_el22'])
data_frame16 = pd.DataFrame(index=[], columns=['sample_mean_mu_el1', 'sample_mean_mu_el2'])
data_frame17 = pd.DataFrame(index=[], columns=['sample_mean_muLambda_el11', 'sample_mean_muLambda_el12', 'sample_mean_muLambda_el21', 'sample_mean_muLambda_el22'])
data_frame18 = pd.DataFrame(index=[], columns=['delta_mu_el1', 'delta_mu_el2'])
data_frame19 = pd.DataFrame(index=[], columns=['delta_muLambda_el11', 'delta_muLambda_el12', 'delta_muLambda_el21', 'delta_muLambda_el22'])
...
for epoch in range(num_epochs):
    ...
    result = sess.run([sample_q_mu_ass, sample_Sigma, lambda_mu_update, lambda_muLambda_cared, lambda_Lambda_cared,
                       eig_val_lambda_Lambda_cared, log_p, log_q, eig_Sigma, lambda_muLambda_update,
                       lambda_Lambda_update, log_p_x, log_p_Lambda, Area_covariance_ellipse,
                       update_counter_ass, rho_ass, grad_q_Lambda, sample_mean_Lambda, delta_Lambda,
                       inv_expect_val_q_Lambda, eig_inv_expect_val_q_Lambda, grad_q_mu,
                       grad_q_muLambda, sample_mean_mu, sample_mean_muLambda, delta_mu, delta_muLambda],
    ...
    series6 = pd.Series([np.mean(result[6]), np.mean(result[7]), elbo], index=data_frame6.columns)
    data_frame6 = data_frame6.append(series6, ignore_index=True)
    ...
    for s in range(num_samples):
        series7 = pd.Series(np.mean(result[13], axis=0), index=data_frame7.columns)
        data_frame7 = data_frame7.append(series7, ignore_index=True)

    for s in range(num_samples):
        series8 = pd.Series([result[16][s][0][0][0], result[16][s][0][0][1], result[16][s][0][1][0], result[16][s][0][1][1]], index=data_frame8.columns)
        data_frame8 = data_frame8.append(series8, ignore_index=True)
    
    series9 = pd.Series([result[17][0][0], result[17][0][1], result[17][1][0], result[17][1][1]], index=data_frame9.columns)
    data_frame9 = data_frame9.append(series9, ignore_index=True)
    
    series10 = pd.Series([result[18][0][0], result[18][0][1], result[18][1][0], result[18][1][1]], index=data_frame10.columns)
    data_frame10 = data_frame10.append(series10, ignore_index=True)

    series11 = pd.Series([result[19][0][0], result[19][0][1], result[19][1][0], result[19][1][1]],
                         index=data_frame11.columns)
    data_frame11 = data_frame11.append(series11, ignore_index=True)

    series12 = pd.Series([result[20][0], result[20][1]], index=data_frame12.columns)
    data_frame12 = data_frame12.append(series12, ignore_index=True)

    series13 = pd.Series([result[5][0], result[5][1]], index=data_frame13.columns)
    data_frame13 = data_frame13.append(series13, ignore_index=True)

    for s in range(num_samples):
        series14 = pd.Series([result[21][s][0][0], result[21][s][0][1]], index=data_frame14.columns)
        data_frame14 = data_frame14.append(series14, ignore_index=True)

    for s in range(num_samples):
        series15 = pd.Series(
            [result[22][s][0][0][0], result[22][s][0][0][1], result[22][s][0][1][0], result[22][s][0][1][1]],
            index=data_frame15.columns)
        data_frame15 = data_frame15.append(series15, ignore_index=True)

    series16 = pd.Series([result[23][0], result[23][1]], index=data_frame16.columns)
    data_frame16 = data_frame16.append(series16, ignore_index=True)

    series17 = pd.Series([result[24][0][0], result[24][0][1], result[24][1][0], result[24][1][1]],
                         index=data_frame17.columns)
    data_frame17 = data_frame17.append(series17, ignore_index=True)

    series18 = pd.Series([result[25][0], result[25][1]], index=data_frame18.columns)
    data_frame18 = data_frame18.append(series18, ignore_index=True)

    series19 = pd.Series([result[26][0][0], result[26][0][1], result[26][1][0], result[26][1][1]],
                         index=data_frame19.columns)
    data_frame19 = data_frame19.append(series19, ignore_index=True)
    ...
    print("delta_Lambda")
    print(result[18])
    print("inv_expect_val_q_Lambda")
    print(result[19])
    print("eig_inv_expect_val_q_Lambda")
    print(result[20])
    print("grad_q_mu")
    print(result[21])
    print("grad_q_muLambda")
    print(result[22])
    print("sample_mean_mu")
    print(result[23])
    print("sample_mean_muLambda")
    print(result[24])
    print("delta_mu")
    print(result[25])
    print("delta_muLambda")
    print(result[26])
...
data_frame8.to_csv("res_grad_q_Lambda_single.csv", index=False)
data_frame9.to_csv("res_sample_mean_Lambda_single.csv", index=False)
data_frame10.to_csv("res_delta_Lambda_single.csv", index=False)
data_frame11.to_csv("res_inv_expect_val_q_Lambda.csv", index=False)
data_frame12.to_csv("res_eig_inv_expect_val_q_Lambda.csv", index=False)
data_frame13.to_csv("res_eig_lambda_Lambda_cared.csv", index=False)
data_frame14.to_csv("res_grad_q_mu_single.csv", index=False)
data_frame15.to_csv("res_grad_q_muLambda_single.csv", index=False)
data_frame16.to_csv("res_sample_mean_mu_single.csv", index=False)
data_frame17.to_csv("res_sample_mean_muLambda_single.csv", index=False)
data_frame18.to_csv("res_delta_mu_single.csv", index=False)
data_frame19.to_csv("res_delta_muLambda_single.csv", index=False)
