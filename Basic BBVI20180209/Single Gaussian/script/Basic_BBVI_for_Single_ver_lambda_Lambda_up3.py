...
#hyper_V = tf.constant([[5.0, 0.0], [0.0, 5.0]])
#hyper_nu = tf.constant(2.05)
hyper_V = tf.constant([[0.15, 0.0], [0.0, 0.2]])
hyper_nu = tf.constant(3.0)
#init_lambda_mu = tf.convert_to_tensor(np.array(rand.multivariate_normal(mean=[0.0, 0.0], cov=[[0.1, 0.0], [0.0, 0.1]]), dtype=np.float32))
#init_lambda_mu = tf.convert_to_tensor(np.array(rand.multivariate_normal(mean=[1.5, 1.5], cov=[[0.1, 0.0], [0.0, 0.1]]), dtype=np.float32))
init_lambda_mu = tf.constant([0.0, 0.0])
#eig_val_random_wishart = tf.self_adjoint_eigvals(tf.convert_to_tensor(np.array(stats.wishart.rvs(df=2.0, scale=[[0.5, 0.0], [0.0, 0.5]]), dtype=np.float32)))
#init_lambda_Lambda = tf.contrib.linalg.LinearOperatorDiag(eig_val_random_wishart, name='init_lambda_Lambda').to_dense()
init_lambda_Lambda = tf.contrib.linalg.LinearOperatorDiag(tf.matrix_diag_part(tf.convert_to_tensor(np.array(stats.wishart.rvs(df=2.0, scale=[[0.1, 0.0], [0.0, 0.1]]), dtype=np.float32)))).to_dense()
#init_lambda_Lambda = tf.convert_to_tensor(np.array(stats.wishart.rvs(df=10.0, scale=[[1.5, 0.0], [0.0, 1.5]]), dtype=np.float32))

#init_lambda_Lambda = tf.constant([[0.22, 0.0], [0.0, 0.22]], dtype=tf.float32)
...
init_lambda_nu = tf.constant(6.0)
...
with tf.name_scope("GenerativeModel"):
    ...
    generative_gauss = tf.contrib.distributions.MultivariateNormalFullCovariance(loc=hyper_alpha_mean,
                                                                                 covariance_matrix=covariance_generative_gauss)  # [S, N, D]
...
with tf.name_scope('LogDistributions'):
    ...
    #log_p_Lambda = tf.reduce_sum(p_Lambda.log_prob(sample_q_Lambda_ass), axis=0, name='log_p_Lambda')
    ...
    log_p = tf.add(log_p_x, log_p_Lambda, name='log_p')
...
for epoch in range(num_epochs):
    ...
    result = sess.run([sample_q_mu_ass, sample_Sigma, lambda_Lambda_cared,
                       eig_val_lambda_Lambda_cared, log_p, log_q, eig_Sigma,
                       lambda_Lambda_update,
                       log_p_x, log_p_Lambda, Area_covariance_ellipse,
                       update_counter_ass, rho_ass, grad_q_Lambda, sample_mean_Lambda, delta_Lambda,
                       inv_expect_val_q_Lambda, eig_inv_expect_val_q_Lambda],
    ...
    series4 = pd.Series(
        [result[2][0][0], result[2][0][1], result[2][1][0], result[2][1][1]], index=data_frame4.columns)
    data_frame4 = data_frame4.append(series4, ignore_index=True)
    ...
    for s in range(num_samples):
        series7 = pd.Series(np.mean(result[10], axis=0), index=data_frame7.columns)
        data_frame7 = data_frame7.append(series7, ignore_index=True)
    
    for s in range(num_samples):
        series8 = pd.Series([result[13][s][0][0][0], result[13][s][0][0][1], result[13][s][0][1][0], result[13][s][0][1][1]], index=data_frame8.columns)
        data_frame8 = data_frame8.append(series8, ignore_index=True)
    
    series9 = pd.Series([result[14][0][0], result[14][0][1], result[14][1][0], result[14][1][1]], index=data_frame9.columns)
    data_frame9 = data_frame9.append(series9, ignore_index=True)
    
    series10 = pd.Series([result[15][0][0], result[15][0][1], result[15][1][0], result[15][1][1]], index=data_frame10.columns)
    data_frame10 = data_frame10.append(series10, ignore_index=True)

    series11 = pd.Series([result[16][0][0], result[16][0][1], result[16][1][0], result[16][1][1]], index=data_frame11.columns)
    data_frame11 = data_frame11.append(series11, ignore_index=True)

    series12 = pd.Series([result[17][0], result[17][1]], index=data_frame12.columns)
    data_frame12 = data_frame12.append(series12, ignore_index=True)

    series13 = pd.Series([result[3][0], result[3][1]], index=data_frame13.columns)
    ...
    print("Area_covariance_ellipse")
    print(result[10])
    #print("inv_sample_p_Lambda")
    #print(result[11])
    #print("inv_sample_q_Lambda_ass")
    #print(result[12])
    #print("cond_counter")
    #print(result[11])
