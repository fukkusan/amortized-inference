...
# Prepare for plot
print("* sample_values for plot *")
sample_mu = q_mu.sample( sample_shape=[num_samples] )
sample_Sigma = tf.matrix_inverse( q_Lambda.sample( sample_shape=[num_samples] ) )
...
cnt = 0
data_frame = pd.DataFrame( index=[], columns=['mu_class1_element1', 'mu_class1_element2', 'mu_class2_element1', 'mu_class2_element2', 'mu_class3_element1', 'mu_class3_element2', 'Lambda_class1_element11', 'Lambda_class1_element12', 'Lambda_class1_element21', 'Lambda_class1_element22', 'Lambda_class2_element11', 'Lambda_class2_element12', 'Lambda_class2_element21', 'Lambda_class2_element22', 'Lambda_class3_element11', 'Lambda_class3_element12', 'Lambda_class3_element21', 'Lambda_class3_element22'] )		# null data frame
sess.run(init)
for epoch in range(3):
    result = sess.run( [sample_mu, sample_Sigma] )		# for plot test
    
    # for debug
    for sam in range(num_samples):
        series = pd.Series( [ result[0][sam][0][0], result[0][sam][0][1], result[0][sam][1][0], result[0][sam][1][1], result[0][sam][2][0], result[0][sam][2][1], result[1][sam][0][0][0], result[1][sam][0][0][1], result[1][sam][0][1][0], result[1][sam][0][1][1], result[1][sam][1][0][0], result[1][sam][1][0][1], result[1][sam][1][1][0], result[1][sam][1][1][1], result[1][sam][2][0][0], result[1][sam][2][0][1], result[1][sam][2][1][0], result[1][sam][2][1][1] ], index=data_frame.columns )
        data_frame = data_frame.append( series, ignore_index=True )
    print(result)
    cnt = cnt + 1
    print(cnt)
data_frame.to_csv("res_mu_and_Lambda.csv", index=False)
