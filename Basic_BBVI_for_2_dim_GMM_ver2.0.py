...
hyper_alpha_mean = tf.constant([[0.0, 0.0], [3.0, 3.0], [-3.0, -3.0]], shape=[K, D], dtype=tf.float32, name='hyper_alpha_mean')
...
with tf.name_scope("GenerativeModel"):
    ...
    #pi_gene = tf.cond(tf.greater(update_counter[0], 0.0), lambda: q_pi.sample(sample_shape=[S]), lambda: p_pi.sample(sample_shape=[S]), name='pi_gene')		# pi for Cat(z|pi) is sampled by p_pi firstly and q_pi secondly
    #pi_gene = tf.cond(tf.equal(pred_counter, 1.0), lambda: q_pi.sample(sample_shape=[S]), lambda: p_pi.sample(sample_shape=[S]), name='pi_gene')  # pi for Cat(z|pi) is sampled by p_pi firstly and q_pi secondly
    pi_gene = q_pi.sample(sample_shape=[S])
    #mu_gene = tf.cond(tf.greater(update_counter[0], 0.0), lambda: q_mu.sample(sample_shape=[S]), lambda: p_mu.sample(sample_shape=[S]), name='mu_gene')		# mu for Normal(x|mu,Lambda) is sampled by p_mu firstly and q_mu secondly
    #mu_gene = tf.cond(tf.equal(pred_counter, 1.0), lambda: q_mu.sample(sample_shape=[S]), lambda: p_mu.sample(sample_shape=[S]), name='mu_gene')  # mu for Normal(x|mu,Lambda) is sampled by p_mu firstly and q_mu secondly
    mu_gene = q_mu.sample(sample_shape=[S])
    #covariance_generative_gauss = tf.cond(tf.greater(update_counter[0], 0.0), lambda: tf.matrix_inverse(q_Lambda.sample(sample_shape=[S])), lambda: tf.matrix_inverse(p_Lambda.sample(sample_shape=[S])))
    #covariance_generative_gauss = tf.cond(tf.equal(pred_counter, 1.0), lambda: tf.matrix_inverse(q_Lambda.sample(sample_shape=[S])), lambda: tf.matrix_inverse(p_Lambda.sample(sample_shape=[S])))
    covariance_generative_gauss = tf.matrix_inverse(q_Lambda.sample(sample_shape=[S]))
    _pi_gene = []
    for ipi in range(N):
        _pi_gene.append(pi_gene)
    pi_gene_tf = tf.reshape(tf.convert_to_tensor(_pi_gene), shape=[S, N, K])
    p_z = tf.contrib.distributions.OneHotCategorical(pi_gene_tf)
    _mu_gene = []
    for imu in range(N):
        _mu_gene.append(mu_gene)
    mu_gene_tf = tf.reshape(tf.convert_to_tensor(_mu_gene), shape=[S, N, K, D])
    _covariance_generative_gauss = []
    for icov in range(N):
        _covariance_generative_gauss.append(covariance_generative_gauss)
    covariance_generative_gauss_tf = tf.reshape(tf.convert_to_tensor(_covariance_generative_gauss), shape=[S, N, K, D, D])
    generative_gauss = tf.contrib.distributions.MultivariateNormalFullCovariance(loc=mu_gene_tf, covariance_matrix=covariance_generative_gauss_tf)
...
with tf. name_scope('Sampling'):
    ...
    #x_gene = tf.reshape([X[:S,:], X[:S,:], X[:S,:]], [S, K, D])
    x_gene = tf.reshape([X[:], X[:], X[:]], shape=[N, K, D])
    _x_gene_sample = []
    for i in range(S):
    ...
    sample_q_Lambda_ass = tf.assign( sample_q_Lambda, q_Lambda.sample( sample_shape=[S], seed=0 ), name='sample_q_Lambda_ass' )
...
with tf.name_scope('LogDistributions'):
    #_log_gene_gauss = []
    #for i in range(N):
    #    _log_gene_gauss.append(generative_gauss.log_prob(sample_gene_gauss_ass, name='log_gene_gauss_element'))
    #log_gene_gauss = tf.reshape(tf.convert_to_tensor(_log_gene_gauss), shape=[S, N, K], name='log_gene_gauss')
...
with tf.name_scope('Gradients'):
    ...
    for j in range(S):
        ...
        grad_q_Lambda.append( tf.gradients(log_q[j], lambda_Lambda, name='grad_q_Lambda')[0] )
...
with tf.name_scope('Care_lambda_muLambda'):
    ...
    for k in range(K):
        _pred_eig_lambda_muLambda.append(tf.to_float(tf.greater(eig_val_lambda_muLambda[k], tf.zeros([D]))))
    for k in range(K):
        for d in range(D):
            _eig_val_lambda_muLambda_modify.append(tf.cond(tf.equal(_pred_eig_lambda_muLambda[k][d], 1.0), lambda:eig_val_lambda_muLambda[k][d], lambda:epsilon))
    ...
    _lambda_muLambda_cared = tf.cond(tf.equal( tf.reduce_sum( pred_lambda_muLambda ), tf.multiply( tf.to_float(K), tf.to_float(D) ) ), lambda: lambda_muLambda_update, lambda: diagonalized_muLambda, name='_lambda_muLambda_cared' )		# Sum of pred_lambda_muLambda is K*D if all eigenvalues are positive.
...
with tf.name_scope('Care_lambda_Lambda'):
    ...
    for k in range(K):
        _pred_eig_lambda_Lambda.append(tf.to_float(tf.greater(eig_val_lambda_Lambda[k], tf.zeros([D]))))
    #pred_eig_lambda_Lambda = tf.convert_to_tensor(_pred_eig_lambda_Lambda)
    for k in range(K):
        for d in range(D):
            _eig_val_lambda_Lambda_modify.append(tf.cond(tf.equal(_pred_eig_lambda_Lambda[k][d], 1.0), lambda:eig_val_lambda_Lambda[k][d], lambda:epsilon))
    ...
    lambda_Lambda_new = tf.matmul(tf.matrix_inverse(eig_vec_lambda_Lambda),
                                  tf.matmul(diagonalized_Lambda, eig_vec_lambda_Lambda),
    ...
    _lambda_Lambda_cared = tf.cond( tf.equal(tf.reduce_sum(pred_lambda_Lambda), tf.multiply( tf.to_float(K), tf.to_float(D))), lambda: lambda_Lambda_update, lambda: lambda_Lambda_new, name='_lambda_Lambda_cared')		# Sum of pred_lambda_Lambda is K*D if all eigenvalues are positive.
    lambda_Lambda_cared = tf.assign(lambda_Lambda, _lambda_Lambda_cared, name='lambda_Lambda_cared')
    assert isinstance(lambda_Lambda_cared, object)
...
for epoch in range(num_epochs):
    ...
    #result = sess.run(
    #    [sample_mu, sample_Sigma, sample_z, sample_pi, lambda_mu_update, lambda_muLambda_cared, lambda_Lambda_cared,
    #     lambda_nu_cared, lambda_z_cared, lambda_pi_cared, eig_val_lambda_Lambda, lambda_muLambda,
    #     eig_val_lambda_muLambda, delta_muLambda, grad_q_muLambda, log_loss, log_p, log_q, log_p_x, log_p_z, log_p_pi,
    #     log_p_mu, log_p_Lambda, sample_q_mu_ass, sample_q_Lambda_ass, sample_q_z_ass, sample_q_pi_ass, delta_mu,
    #     delta_Lambda, delta_nu, delta_pi, delta_z], feed_dict={X: x})  # for plot test
    result = sess.run(
        [sample_mu, sample_Sigma, sample_z, sample_pi, lambda_mu_update, lambda_muLambda_cared, lambda_Lambda_cared,
         lambda_nu_cared, lambda_z_cared, lambda_pi_cared, eig_val_lambda_Lambda, lambda_Lambda_update,
         eig_val_lambda_muLambda, lambda_muLambda_update, lambda_nu_update, lambda_z_update, lambda_pi_update,
         grad_q_mu, grad_q_muLambda, grad_q_Lambda, grad_q_nu, grad_q_z, grad_q_pi, log_loss, log_p, log_q, log_p_x,
         log_p_z, log_p_pi, log_p_mu, log_p_Lambda, log_q_z, log_q_pi, log_q_mu, log_q_Lambda, sample_q_mu_ass,
         sample_q_Lambda_ass, sample_q_z_ass, sample_q_pi_ass, delta_mu, delta_muLambda, delta_Lambda, delta_nu,
    ...
    #result = sess.run(eig_val_lambda_Lambda_modify, feed_dict={X:x})
    ...
    print(result)
    #print(result[11], result[12], result[13])  # lambda_muLambda_update, eig_val_lambda_muLambda, delta_muLambda
    #print(result[14], result[15])  # grad_q_muLambda and log_loss
    #print(result[16], result[17])  # log_p and log_q
    #print(result[18], result[19], result[20], result[21], result[22])  # log_p_x, log_p_z, log_p_pi, log_p_mu, log_p_Lambda
    #print(result[23], result[24])  # sample_q_mu_ass, sample_q_Lambda_ass
    #print(result[23], result[24], result[25],result[26])  # sample_q_mu_ass, sample_q_Lambda_ass, sample_q_z_ass, sample_q_pi_ass
    #print(result[27], result[13], result[28], result[29], result[30], result[31]) # delta_mu, delta_muLambda, delta_Lambda, delta_nu, delta_pi, delta_z
    cnt = cnt + 1
    print(cnt)
    #log_writer = tf.summary.FileWriter('Graph_BBVI2', sess.graph)
