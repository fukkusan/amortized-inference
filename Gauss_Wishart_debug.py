...
sample_q_mu_ass = tf.assign( sample_q_mu, q_mu.sample(sample_shape=[S], seed = 0) )
sample_q_Lambda = tf.Variable( tf.ones([S, K, D, D]) )
sample_q_Lambda_ass = tf.assign( sample_q_Lambda, q_Lambda.sample(sample_shape=[S], seed = 0) )
...
sess.run(lambda_mu_update)
#sess.run(lambda_muLambda_update)
